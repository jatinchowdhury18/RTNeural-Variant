...
script_file=RTNeuralVariant.pkgproj
...
pkg_dir=RTNeuralVariant_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/RTNeuralVariant.pkg $pkg_dir/RTNeuralVariant-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/RTNeuralVariant-signed.pkg --bundle-id com.chowdsp.RTNeuralVariant --asc-provider "$TEAM_ID" --username chowdsp@gmail.com --password "$INSTALLER_PASS"

echo "Building disk image..."
vol_name=Install_RTNeuralVariant-$app_version
rm -f "$vol_name"
