...
mkdir -p bin/Win64
mkdir -p bin/Win32
declare -a plugins=("RTNeuralVariant")
...
(
    cd bin
    rm -f "RTNeuralVariant-Win64-${VERSION}.zip"
    zip -r "RTNeuralVariant-Win64-${VERSION}.zip" Win64
    # rm -f "RTNeuralVariant-Win32-${VERSION}.zip"
    # zip -r "RTNeuralVariant-Win32-${VERSION}.zip" Win32
)
